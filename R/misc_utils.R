...
# qs / qs2 compatibility: read whichever of .qs2 / .qs exists, always save as qs2.
...
GetNetsNameString <- function(){
  # net.stats is created when a network is decomposed into modules; before that
  # there are no networks to name, so return an empty string rather than raise
  # "object 'net.stats' not found". This copy must stay self-contained: some
  # modules load misc_utils.R without utils_coexp_net.R.
